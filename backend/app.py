"""
backend/app.py - COMPLETE VERSION v4.5.5
...
CHANGES IN v4.5.5:
- FIXED: Live Greeks Monitor Discord alerter error
- Disabled Discord alerts for Live Greeks (prevents spam every 10 seconds)
- Live Greeks Monitor still tracks data, just doesn't send Discord alerts

CHANGES IN v4.5.4:
- CRITICAL FIX: Added environment variable expansion for config.yaml
- Fixes ${DISCORD_MOMENTUM_SIGNALS} and all other ${...} variables
- Added expand_env_vars_in_dict() function to recursively expand env vars
...
# ========================================
# CRITICAL FIX: Environment Variable Expansion for config.yaml
# ========================================

def expand_env_vars_in_dict(config_dict):
    """
    Recursively expand ${VAR_NAME} environment variables in config dictionary
    
    This fixes the issue where ${DISCORD_MOMENTUM_SIGNALS} was being treated
    as a literal string instead of being replaced with the actual webhook URL.
    
    Args:
        config_dict: Dictionary, list, or string to process
    
    Returns:
        Processed object with environment variables expanded
    """
    if isinstance(config_dict, dict):
        return {key: expand_env_vars_in_dict(value) for key, value in config_dict.items()}
    elif isinstance(config_dict, list):
        return [expand_env_vars_in_dict(item) for item in config_dict]
        ...
        def replace_env_var(match):
            ...
            if env_value is None:
                logger.warning(f"⚠️ Environment variable ${{{var_name}}} not found, keeping original value")
...
try:
    ...
    # CRITICAL: Expand ${VAR_NAME} environment variables
    config_yaml = expand_env_vars_in_dict(config_yaml)
    
    logger.info("✅ config.yaml loaded with environment variables expanded")
...
POLYGON_KEY = POLYGON_API_KEY  # Alias for compatibility
or_analyzer = None
...
if LIVE_GREEKS_AVAILABLE and analyzer:
    try:
        # FIXED: Disable Discord alerts for Live Greeks (prevents spam + webhook errors)
        # Live Greeks update every 10 seconds - Discord alerts would be too noisy
        live_greeks_monitor = LiveGreeksMonitor(
            analyzer=analyzer,
            discord_alerter=None,  # Disabled to prevent 'webhook_momentum_signals' errors
            check_interval=10  # 10 seconds
        )
        logger.info("✅ Live Greeks Monitor initialized (Discord alerts disabled)")
    except Exception as e:
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("🚀 STARTING PROFESSIONAL TRADING DASHBOARD v4.5.5")
    print("   (WITH ENV VAR FIX + LIVE GREEKS - NO SPAM)")
    print("=" * 60)
    ...
    if live_greeks_monitor:
        ...
        print(f"   📡 Data tracked (Discord alerts disabled)")
    ...
    print(f"   • Live Greeks Monitor - Real-time options greeks (no alerts)")
    ...
    print(f"   • 🔧 ENV VAR FIX - Discord webhooks work! ✅")
    print(f"   • 🔧 GREEKS FIX - No more spam errors! ✅")
    print("=" * 60 + "\n")
